...
def srr_web_scrapping(list_srx_address):
    '''Receives a list of Srx url address.
    Returns a list of list with each SRX and
    its SRR associated'''
...
def df_srx_srr(list_srx_srr):
    '''Receives a list of list
    (SRR_srx_ncbi) and returns a
    dataframe with SRX and SRR info
    '''
...
def merge_dfs(df1, df2):
    '''Function to merge the first df
    (df with GSM and Srx url columns)
    and df_srr with the
    SRX and SRR columns'''
...
def save_df(df, output_file_name):
    '''Function to save the final df
    into a csv file containing the GSM,
    Srx (address) and SRR information
    per sample (GSM)'''
